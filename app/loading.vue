...
// ======= SCENE =======
function buildScene(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, theme: Theme): Scene {
  ...
  const particles: Particle[] = Array.from({ length: conf.PARTICLE_COUNT }, () => {
    const p = createParticle(vw, vh)
    p.born = Math.random() * p.maxLife
    return p
  })
...
function createParticle(vw: number, vh: number): Particle {
  return {
    ...
    r: Math.random() * 2 + 0.5, maxLife: 3000 + Math.random() * 4000, born: 0,
  }
}
function resetParticle(p: Particle, vw: number, vh: number) {
  Object.assign(p, createParticle(vw, vh))
}
function updateParticle(p: Particle, dt: number, vw: number, vh: number) {
  p.x += p.vx * dt; p.y += p.vy * dt; p.born += dt
  if (p.born > p.maxLife || p.x < -20 || p.x > vw + 20 || p.y < -20 || p.y > vh + 20)
    resetParticle(p, vw, vh)
}
function drawParticle(ctx: CanvasRenderingContext2D, p: Particle, theme: Theme, alpha: number) {
  ...
  ctx.shadowColor = `hsla(${hue},90%,65%,${a * 0.6})`; ctx.shadowBlur = 8
...
function createStream(edges: Edge[], streamLen: number, theme: Theme): Stream {
  const s: Stream = {
    hueOff: Math.random() * theme.hueRange,
    path: [edges[Math.floor(Math.random() * edges.length)]!],
    t: Math.random(), speed: 0.01 + Math.random() * 0.008, streamLen,
  }
...
function drawGrid(ctx: CanvasRenderingContext2D, theme: Theme, alpha: number, now: number, sc: Scene) {
  ...
  for (const h of sc.hexes) {
    ...
    ctx.globalAlpha = (1 - (h.dist / sc.maxDist) * 0.3) * alpha
...
function handleResize() {
  ...
  resizeTimer = setTimeout(() => {
    if (canvasRef.value && _ctx) _sc = buildScene(canvasRef.value, _ctx, getTheme(isDark.value))
  }, 150)
}
...
function frame(now: number) {
  ...
  loadingText.value = 'Loading' + '.'.repeat(Math.floor(now / 400) % 4)
  ctx.clearRect(0, 0, sc.vw, sc.vh)
  drawGrid(ctx, theme, 1, now, sc)
  drawAmbient(ctx, theme, now, 1, sc)
  drawBgLogo(ctx, theme, now, 1, sc)
  for (const p of sc.particles) { updateParticle(p, dt, sc.vw, sc.vh); drawParticle(ctx, p, theme, 1) }
  for (const s of sc.streams) { updateStream(s, dt); drawStream(ctx, s, theme, 1, now) }
  drawOrbital(ctx, theme, now, 1, sc)
  drawEmblem(ctx, theme, now, 1, sc)
...
onMounted(() => {
  ...
  _sc = buildScene(canvas, _ctx, getTheme(isDark.value))
...
<template>
  ...
  >
    ...
    <!-- Progress overlay -->
    <div
      class="absolute left-1/2 -translate-x-1/2 flex flex-col items-center gap-3 px-6 py-4 rounded-xl backdrop-blur-md border"
      style="bottom: 8%"
      :class="isDark
    ...
    >
      ...
      >
        <div
          class="h-full rounded transition-[width] duration-300 ease-out"
          :class="isDark
      ...
      <span
        ...
        :class="isDark ? 'text-[rgba(74,245,255,0.8)]' : 'text-[rgba(59,130,246,0.9)]'"
      >{{ Math.floor(Math.max(progress, 0)) }}%</span>
    </div>
  </div>
</template>
